...
require_once($_SERVER['DOCUMENT_ROOT'] . '/ged/roll/cm/include/html_header.php');

require_once($_SERVER['DOCUMENT_ROOT'] . '/ged/roll/cm/include/header.php');

require_once($_SERVER['DOCUMENT_ROOT'] . '/ged/roll/cm/include/sidebar.php');
                                ...
                                <div class="d-flex justify-content-between align-items-start flex-wrap mb-2">
                                    ...
                                    <div class="d-flex flex-column">
                                        ...
                                        <div class="d-flex align-items-center mb-2">
                                            <a id="nom_client" href="roll/cm/clients/view" class="text-gray-900 text-hover-primary fs-2 fw-bold me-1">--</a>
                                        </div>
                                        <!--end::Name-->
                                        <!--begin::Info-->
                                        ...
                                        <div class="col-md-4 rounded-3 p-3 mb-5">
                                            <a id="view_facture_total_echue_query" href="#">
                                                <div class="card card-flush flex-column flex-stack py-5" style="background: linear-gradient(#f1416c 60%, #f5f8fa);">
                                                    <div class="text-white text-center fs-2 fw-bold">Factures échues</div>
                                        ...
                                        <div class="col-md-4 rounded-3 p-3 mb-5">
                                            <a id="view_facture_total_en_cour_query" href="#">
                                                <div class="card card-flush bg-primary flex-column flex-stack py-5" style="background: linear-gradient(#009ef7 60%, #f5f8fa);">
                                                    <div class="text-white text-center fs-2 fw-bold">Factures non échues</div>
                                                    <div class="text-center">
...
<?php require_once($_SERVER['DOCUMENT_ROOT'] . '/ged/roll/cm/include/footer_activities.php'); ?>
...
<?php require_once($_SERVER['DOCUMENT_ROOT'] . '/ged/roll/cm/include/pages_script.php'); ?>
...
<script>
    $(document).ready(function () {
        ...
        function reload_datatable1() {
            $.ajax({
                url: "roll/cm/dossiers/fetch.php",
                method: "POST",
                data: {
        ...
        function reload_page() {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#view_facture_total_facture_query').attr('href', 'roll/cm/comptabilite/facture?query=' + query_total_facture);
                    $('#view_facture_total_regle').html(total_regle);
                    $('#view_facture_total_regle_query').attr('href', 'roll/cm/comptabilite/facture?query=' + query_total_regle);

                    $('#view_stat_contrat').html(stat_contrat);
                    $('#view_stat_contrat_query').attr('href', 'roll/cm/comptabilite/facture?query=' + query_stat_contrat);
                    $('#view_stat_facture').html(stat_facture);
                    $('#view_stat_facture_query').attr('href', 'roll/cm/comptabilite/facture?query=' + query_stat_facture);
                    $('#view_stat_non_facture').html(stat_non_facture);
                    $('#view_stat_non_facture_query').attr('href', 'roll/cm/comptabilite/facture?query=' + query_stat_non_facture);
                    $('#view_stat_encaisse').html(stat_encaisse);
                    $('#view_stat_encaisse_query').attr('href', 'roll/cm/comptabilite/facture?query=' + query_stat_encaisse);
                    $('#view_stat_creance').html(stat_creance);
                    $('#view_stat_creance_query').attr('href', 'roll/cm/comptabilite/facture?query=' + query_stat_creance);
        ...
        function date_formatter(date, format) {
            ...
            } else if (format == 'mm/cm/yyyy') {
                return [month, day, year].join('/');
            } else if (format == 'yyyy/mm/cm') {
                return [year, month, day].join('/');
            } else if (format == 'yyyy/cm/mm') {
            ...
            } else if (format == 'mm/cm/yyyy hh:mm') {
                return [month, day, year].join('/') + ' ' + hour + ':' + minute;
            } else if (format == 'yyyy/mm/cm hh:mm') {
                return [year, month, day].join('/') + ' ' + hour + ':' + minute;
            } else if (format == 'yyyy/cm/mm hh:mm') {
            ...
            } else if (format == 'mm/cm/yyyy hh:mm:ss') {
                return [month, day, year].join('/') + ' ' + hour + ':' + minute + ':' + second;
            } else if (format == 'yyyy/mm/cm hh:mm:ss') {
                return [year, month, day].join('/') + ' ' + hour + ':' + minute + ':' + second;
            } else if (format == 'yyyy/cm/mm hh:mm:ss') {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#view_facture_total_echue').html(total_echue);
                $('#view_facture_nb_echue').html('(' + nb_facture_echue + ')');
                $('#view_facture_total_echue_query').attr('href', 'roll/cm/comptabilite/facture?query=' + query_total_echue);
                $('#view_facture_total_en_cour').html(total_en_cour);
                $('#view_facture_nb_en_cour').html('(' + nb_facture_en_cour + ')');
                $('#view_facture_total_en_cour_query').attr('href', 'roll/cm/comptabilite/facture?query=' + query_total_en_cour);
                $('#view_facture_total_solde').html(total_solde);
                $('#view_facture_nb_solde').html('(' + nb_facture_solde + ')');
                $('#view_facture_total_solde_query').attr('href', 'roll/cm/comptabilite/facture?query=' + query_total_solde);
        ...
        // fetch data secteur d'activité pour le formulaire de modification client
        $.ajax({
            url: "roll/cm/dossiers/fetch.php",
            method: "POST",
            data: {
                                ...
                                $.ajax({
                                    ...
                                    success: function (data) {
                                        ...
                                        var myDropzone = new Dropzone(id, { // Make the whole body a dropzone
                                            url: "roll/cm/dossiers/fetch.php?titre_document=" + data.titre_document + "&id_document=" + id_document + "&action=doc_file_upload", // Set the url for your upload script location
                                            parallelUploads: 20,
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    var myDropzone = new Dropzone(id1, { // Make the whole body a dropzone
                        url: "roll/cm/dossiers/fetch.php?titre_document=" + data.titre_document + "&id_document=" + id_document + "&action=doc_scan_upload", // Set the url for your upload script location
                        parallelUploads: 20,
                        maxFilesize: 10, // Max filesize in MB
                        maxFiles: 1,
                        previewTemplate: previewTemplate,
