...
foreach ($result as $row) {
    ...
    $date_echeance_plus_5_jours = date('Y-m-d H:i:s', strtotime($row['date_echeance'] . ' +5 days'));
    $date_echeance_plus_30_jours = date('Y-m-d H:i:s', strtotime($row['date_echeance'] . ' +30 days'));
    ...
    if ($row['relance_option_facture'] != 'after_5_days' && $date_echeance_plus_5_jours < date('Y-m-d H:i:s')) {
        ...
        $nom_client = find_info_client('nom_utilisateur', $row['id_client'], $db);
        $mail_html = '<h1>hello</h1>';
        $url = "";
        ...
        $subject = 'Relance pour facture échue';
